...
/* ADC State */
static u16 s_u16LastAdcVal = 0xFFFFU;
static u32 s_u32Multiplier = 1UL;
static u8  s_u8OverrideAdc = 0U; /* Set to 1 if user typed a custom freq */
...
static void APP_voidLoadSettings(void)
{
    ...
    if (local_u8Valid == EEPROM_VALID_FLAG_VAL) {
        ...
        s_u8OverrideAdc = 1U; /* Assume manual load overrides pot initially */
    }
}
...
static void APP_voidUpdateDisplay(void)
{
    ...
    if (s_u32Multiplier == 1UL)       HLCD_voidSendString(" x1   ");
    else if (s_u32Multiplier == 10UL) HLCD_voidSendString(" x10  ");
    else                              HLCD_voidSendString(" x100 ");
...
int main(void)
{
    ...
    while (1) {
        ...
        if (MTIMER_u8IsIntervalElapsed(&local_u32LastLoopMs, UI_UPDATE_PERIOD_MS) == OK) {
            ...
            /* ========================================================
             *  ADC Potentiometer Frequency Sweep
             * ======================================================== */
            if (ADC_enumReadChannel(ADC_CHANNEL_0, &local_u16AdcVal) == ADC_OK) {
                
                /* Check if potentiometer moved significantly (Hysteresis) */
                if (abs((int)local_u16AdcVal - (int)s_u16LastAdcVal) > ADC_HYSTERESIS) {
                    
                    /* Potentiometer moved! Regain control from manual typing override */
                    s_u8OverrideAdc = 0U;
                    s_u16LastAdcVal = local_u16AdcVal;
                    
                    /* Map 0-1023 to 1-1024, multiply by scale */
                    u32 local_u32NewFreq = ((u32)local_u16AdcVal + 1UL) * s_u32Multiplier;
            ...
            if (local_u8Key != KPD_NOT_PRESSED) {
                ...
                /* Enter exact frequency */
                    ...
                    if (s_u8IsTyping) {
                        ...
                        s_u8IsTyping = 0U;
                        s_u8OverrideAdc = 1U; /* Lock ADC out until it moves again */
                ...
                else if (local_u8Key == '+') { s_u32Multiplier = 1UL;   s_u16LastAdcVal = 0xFFFF; s_u8DisplayDirty = 1U; }
                else if (local_u8Key == '-') { s_u32Multiplier = 10UL;  s_u16LastAdcVal = 0xFFFF; s_u8DisplayDirty = 1U; }
                else if (local_u8Key == '*') { s_u32Multiplier = 100UL; s_u16LastAdcVal = 0xFFFF; s_u8DisplayDirty = 1U; }
